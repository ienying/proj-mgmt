...
// GET /api/issue-config/dispatchers
export async function GET() {
  try {
    const client = await createServerClient();
    const { data, error } = await client.rpc("dp_select", {
      p_table: "issue_mgmt_dispatchers",
    });
    if (error) {
...
// POST /api/issue-config/dispatchers
...
// PUT /api/issue-config/dispatchers?id=xxx
...
// DELETE /api/issue-config/dispatchers?id=xxx

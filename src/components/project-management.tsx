...
export function ProjectManagement({
  ...
  // 项目详情面板
  // 项目任务列表
  const [projectTasks, setProjectTasks] = useState<Record<string, unknown>[]>([]);
  const [loadingTasks, setLoadingTasks] = useState(false);

  const loadProjectTasks = useCallback(async (projectId: string) => {
    setLoadingTasks(true);
    try {
      const res = await fetch(`/api/todo-tasks/instances?project_id=${projectId}`);
  ...
  }, [selectedProject, loadProjectTasks, loadProjectMembers]);

  const handleTaskStatusChange = async (taskId: string, newStatus: string) => {
    try {
      const res = await fetch(`/api/todo-tasks/instances/${taskId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "completed": return <Flag className="w-3.5 h-3.5 text-green-500" />;
      ...
      <div className="w-80 shrink-0 h-full border-l border-slate-200 overflow-y-auto">
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-slate-500">任务列表</h3>
            <span className="text-xs text-slate-400">{projectTasks.length} 项</span>
          </div>
          ...
          {loadingTasks ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-slate-400 mt-1">可在待办任务中发布任务</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {isLate && (
                            <span className="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-600">迟交</span>
                          )}
                          {(task.period_label as string) && (
                            <span className="text-xs text-slate-400">{String(task.period_label)}</span>
                          )}
                        </div>
                          ...
                          <div className="flex items-center gap-1.5 mt-2">
                            {taskStatus === "pending" && (
                              <Button
                                ...
                                onClick={() => handleTaskStatusChange(String(task.id), "in_progress")}
                              >
                                开始执行
                              </Button>
                            )}
                            {(taskStatus === "in_progress" || taskStatus === "overdue") && (
                              <Button
                                size="sm"
                                className="h-6 text-xs px-2 bg-green-600 hover:bg-green-700 text-white"
                                onClick={() => handleTaskStatusChange(String(task.id), "completed")}
                              >
                                标记完成
                              </Button>
                            )}

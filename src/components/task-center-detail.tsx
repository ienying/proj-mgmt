...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  completed: { label: "已完成", color: "bg-[#edf8f7] text-[#0d9488]" },
...
export default function TaskCenterDetail({ instance, def, currentUser, onRefresh, onBack }: DetailProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#f4f7fb]" style={{fontFamily:"'PingFang SC','Microsoft YaHei',-apple-system,BlinkMacSystemFont,sans-serif"}}>
      {/* 顶部标题栏 */}
      <div className="shrink-0 bg-white px-8 py-3 flex items-center justify-between" style={{borderBottom:"2px solid #0f2840"}}>
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="py-[7px] px-3 text-xs font-bold text-[#3d5468] cursor-pointer bg-transparent border-2 border-[#0f2840] hover:bg-[#0d2137] hover:text-white transition-all" style={{fontFamily:"inherit"}}>
            ← 返回
          </button>
          <h2 className="text-[22px] font-extrabold text-[#0d2137] -tracking-[0.3px]">{def?.task_name}</h2>
          <span className={`text-[10px] px-2 py-0.5 font-semibold uppercase tracking-[0.3px] border ${
            instance?.status === "completed" ? "border-[#0d9488] text-[#0d9488]"
            : instance?.status === "in_progress" ? "border-[#2563eb] text-[#2563eb]"
            : "border-[#0f2840] text-[#0d2137]"
          }`}>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-8 py-5 space-y-4">
                  ...
                  return (
                    <React.Fragment key={node.id}>
                      <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-none text-[11px] whitespace-nowrap shrink-0 font-medium
                        ${isActive ? "bg-[#eef4ff] text-[#2563eb] ring-1 ring-[#2563eb]/30" :
                          isDone ? "bg-[#edf8f7] text-[#0d9488]" :
            ...
            {/* 任务摘要卡片 */}
            <div className="bg-white p-4" style={{border:"2px solid #0f2840"}}>
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <span>
                    <Badge className={`text-[10px] ${def?.task_mode === "process" ? "text-[#2563eb]" : "bg-purple-50 text-purple-600"} border-0`}>
            ...
            {isSummaryNode && (
              <div className="bg-white rounded-none border border-blue-200 p-4">
                ...
                {collectionLoading ? (
                  ...
                  <div className="border border-[#c0c4cc] overflow-hidden">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 border-b border-[#d0d4da]">
            ...
            <div className="flex justify-between pt-3 border-t border-gray-200 sticky bottom-0 bg-white">
                ...
                {isHandler && (instance?.status === "in_progress" || instance?.status === "pending") && (
                  <>
                    <button onClick={handleSaveDraft} disabled={loading}
                      className="py-[9px] px-4 border-2 border-[#0f2840] bg-white text-xs font-bold text-[#3d5468] cursor-pointer hover:bg-[#0d2137] hover:text-white transition-all disabled:opacity-50"
                      style={{fontFamily:"inherit"}}>暂存草稿</button>
                    <button onClick={handleSubmit} disabled={loading}
                      className="py-[9px] px-4 border-2 border-[#0d2137] bg-[#0d2137] text-white text-xs font-bold cursor-pointer hover:bg-[#2563eb] hover:border-[#2563eb] transition-all disabled:opacity-50"
                      style={{fontFamily:"inherit"}}>提交</button>
                  </>
                )}
              ...
              <button onClick={() => onBack()}
                ...
                style={{fontFamily:"inherit"}}>关闭</button>
            </div>
              ...
              <DialogContent className="max-w-3xl">
                ...
                <div className="space-y-4 mt-2">
                  <div className="border border-gray-200 rounded-none overflow-auto max-h-64">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 border-b border-[#d0d4da] sticky top-0">
